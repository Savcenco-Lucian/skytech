...
const Testimonials = ({ language, translations }) => {
  ...
  const { testTitle, testSub1, testSub2, testSub3, testPar, testimonials
            } = translations[language];

  
  return (
    <section className="px-7 py-20 lg:px-20 bg-[#F2F9FF]">
      <div className='flex flex-col justify-between xl:items-center xl:flex-row'>
        <div className='flex flex-col gap-4 py-7'>
          <h3 className='font-medium text-[#0A0D31] text-lg lg:text-xl'>{testTitle}</h3>
          <SmoothReveal><h2 className='text-[#0A0D31] font-bold text-3xl leading-[45px] xl:text-5xl xl:leading-[65px]'>{testSub1} <span className='bg-gradient-to-r from-[#4491E1] via-[#4491E1] to-[#5D72D9] bg-clip-text text-transparent'>{testSub2} </span> {testSub3} <br /></h2></SmoothReveal>
        </div>
        <div className=''>
          <SmoothReveal><p className='xl:w-[650px] text-[#383C5A] text-base xl:text-lg'>{testPar}
          </p></SmoothReveal>
        </div>
      </div>
      <SmoothReveal><div className="py-7 ">
        <ul className="grid items-center gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((item, idx) => (
            ...
            >
              ...
              <figure>
                <blockquote>
                  <p className="text-[#383C5A] tracking-tighter font-geist text-lg font-normal px-4 py-4">
                    {item.quote}
                  </p>
                </blockquote>
                <hr />
                <div className="flex items-center gap-x-4 p-4 mt-1 bg-page-gradient">

                  <div>
                    <span className="block text-[#0A0D31] font-semibold">
                      {item.name}
                    </span>
                    <span className="block text-[#0A0D31] text-sm mt-0.5">
                      {item.title}
                    </span>
                  </div>
                </div>
